describe('結果ページ下部の手続き詳細部分に表示される要素がシミュレーションのロジックに合致することのテスト', () => {
  it('Q7で国保の加入の検討を選択した場合、国民健康保険への加入手続きのための詳細が表示される', () => {
    ...
    cy.get('#todo-container')
      .find('#todo-health-insurance-plate')
      .should('have.text', '健康保険');
    cy.get('#health-insurance')
      .should('be.visible')
      .find('h5')
      .should('have.text', '国民健康保険への加入手続き');
  });
  ...
  it('Q7で任意継続の検討を選択した場合、任意継続被保険者となる手続きのための詳細が表示される', () => {
    ...
    cy.get('#health-insurance')
      .should('be.visible')
      .find('h5')
      .should('have.text', '任意継続被保険者になる手続き');
  });
  ...
  it('国民年金への加入手続きのための詳細が表示される', () => {
    ...
    cy.visit('/result');
    cy.get('#todo-container')
      .find('#todo-pension-plate')
      .should('have.text', '年金');
    cy.get('#pension')
      .should('be.visible')
      .find('h5')
      .should('have.text', '国民年金への加入手続き');
  });
  ...
  it('自己都合の退職かつ退職予定日までの合計で1年以上の雇用保険の被保険者期間がある場合、雇用保険の失業給付の受給手続きのための詳細が表示される', () => {
    ...
    cy.visit('/result');
    cy.get('#todo-container')
      .find('#todo-employment-insurance-plate')
      .should('have.text', '雇用保険');
    cy.get('#employment-insurance')
      .should('be.visible')
      .find('h5')
      .should('have.text', '雇用保険の失業給付の受給手続き');
  });
  ...
  it('再就職の予定がなく、住民税を普通徴収で納める場合は税金の詳細として住民税の支払いと所得税の還付申請が表示される', () => {
    ...
    cy.get('#tax')
      ...
      .find('#resident-tax')
      .find('h5')
      .should('have.text', '住民税の支払い');
    cy.get('#tax')
      .should('be.visible')
      .find('#income-tax')
      .find('h5')
      .should('have.text', '所得税の還付申請');
  });
