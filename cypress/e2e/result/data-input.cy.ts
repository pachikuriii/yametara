describe('入力内容確認のモーダルが回答内容と合致するかテスト', () => {
  it('質問ページ入力した内容が入力内容としてモーダルに表示される', () => {
    ...
    cy.get('#re-employment').should('have.text', '再就職：年内の予定は未定');
    cy.get('#age').should('have.text', '年齢：30歳未満');
    cy.get('#postcode').should('have.text', '郵便番号：655-0873');
    cy.get('#family').should('have.text', '家族：被保険者の家族はいない');
    cy.get('#emp-ins-last-two-years').should(
      'have.text',
      '過去2年：6ヶ月以上1年未満の加入',
    );
    cy.get('#emp-ins-total').should('have.text', '通算：5年以上10年未満の加入');
    cy.get('#health-ins-last-two-month').should(
      'have.text',
      '健康保険：退職予定日時点で2ヶ月未満の加入',
    );
    cy.get('#health-ins-after-retirement').should(
      'have.text',
      '退職後に加入を検討したい健康保険：国民健康保険',
    );
    cy.get('#tax-after-retirement').should(
      'have.text',
      '退職後の住民税の支払い方法：普通徴収',
    );
  });
